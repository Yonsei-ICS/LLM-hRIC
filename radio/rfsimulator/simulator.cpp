...
static paramdef_t rfsimu_param(const char *name, const char *help, unsigned int flags, char type)
{
  paramdef_t param = {};
  snprintf(param.optname, sizeof(param.optname), "%s", name);
  param.helpstr = help;
...
static paramdef_t rfsimu_string_param(const char *name, const char *help, unsigned int flags, char **ptr, const char *defval)
{
  paramdef_t param = rfsimu_param(name, help, flags, TYPE_STRING);
  param.strptr = ptr;
  param.defstrval = defval;
  return param;
}

static paramdef_t rfsimu_strlist_param(const char *name, const char *help, unsigned int flags, char **ptr, char **defval)
{
  paramdef_t param = rfsimu_param(name, help, flags, TYPE_STRINGLIST);
  param.strlistptr = ptr;
  param.defstrlistval = defval;
  return param;
}

static paramdef_t rfsimu_double_param(const char *name, const char *help, unsigned int flags, double *ptr, double defval)
{
  paramdef_t param = rfsimu_param(name, help, flags, TYPE_DOUBLE);
  param.dblptr = ptr;
  param.defdblval = defval;
  return param;
}

static paramdef_t rfsimu_int_param(const char *name, const char *help, unsigned int flags, int32_t *ptr, int defval)
{
  paramdef_t param = rfsimu_param(name, help, flags, TYPE_INT);
  param.iptr = ptr;
  param.defintval = defval;
  return param;
}

static paramdef_t rfsimu_uint16_param(const char *name, const char *help, unsigned int flags, uint16_t *ptr, uint16_t defval)
{
  paramdef_t param = rfsimu_param(name, help, flags, TYPE_UINT16);
  param.u16ptr = ptr;
  param.defuintval = defval;
  return param;
}

static paramdef_t rfsimu_uint64_param(const char *name, const char *help, unsigned int flags, uint64_t *ptr, uint64_t defval)
{
  paramdef_t param = rfsimu_param(name, help, flags, TYPE_UINT64);
  param.u64ptr = ptr;
  param.defint64val = defval;
  return param;
}
...
#define RFSIMULATOR_PARAMS_DESC {					\
  rfsimu_string_param(RFSIMU_SERVER_ADDR,       "<ip address to connect to>\n",             simOpt, NULL, "127.0.0.1"),            \
  rfsimu_uint16_param(RFSIMU_SERVER_PORT,       "<port to connect to>\n",                   simOpt, NULL, PORT),                   \
  rfsimu_strlist_param(RFSIMU_OPTIONS_PARAMNAME, RFSIM_CONFIG_HELP_OPTIONS,                 simOpt, NULL, NULL),                   \
  rfsimu_string_param(RFSIMU_IQFILE,            "<file path to use when saving IQs>\n",     simOpt, NULL, "/tmp/rfsimulator.iqs"), \
  rfsimu_string_param(RFSIMU_MODELNAME,         "<channel model name>\n",                   simOpt, NULL, "AWGN"),                 \
  rfsimu_double_param(RFSIMU_PLOSS,             "<channel path loss in dB>\n",              simOpt, NULL, 0),                      \
  rfsimu_double_param(RFSIMU_FORGETFACT,        "<channel forget factor ((0 to 1)>\n",      simOpt, NULL, 0),                      \
  rfsimu_uint64_param(RFSIMU_OFFSET,            "<channel offset in samps>\n",              simOpt, NULL, 0L),                     \
  rfsimu_double_param(RFSIMU_PROP_DELAY,        "<propagation delay in ms>\n",              simOpt, NULL, 0.0),                    \
  rfsimu_int_param(RFSIMU_WAIT_TIMEOUT,         "<wait timeout if no UE connected>\n",      simOpt, NULL, 1),                      \
  rfsimu_int_param(RFSIMU_ENABLE_BEAMS,         "<enable simplified beam simulation>\n",    simBool, NULL, 0),                     \
  rfsimu_int_param(RFSIMU_NUM_CONCURRENT_BEAMS, "<number of concurrent beams supported>\n", simOpt, NULL, 1),                      \
  rfsimu_uint64_param(RFSIMU_BEAM_MAP,          "<initial beam map>\n",                     simOpt, NULL, 1),                      \
  rfsimu_string_param(RFSIMU_BEAM_IDS,          "<initial beam ids>\n",                     simOpt, NULL, NULL),                   \
  rfsimu_string_param(RFSIMU_BEAM_GAINS,        "<beam gain matrix in toeplitz form>\n",    simOpt, NULL, NULL),                   \
};
// clang-format on
