...
bool check_pdcp_bearer(pdcp_radio_bearer_stats_t* rb)
{
  ...
  /*
   * Packet and byte counters are sampled while the PDCP data path updates
   * them independently.  A snapshot may therefore temporarily contain the
   * new packet count and the previous byte count.  The 32-bit byte counters
   * can also wrap before their packet counterparts.  Neither condition is a
   * malformed indication, and monitoring data must never terminate the E2
   * node.  Consumers calculate deltas and discard reset/wrap samples.
   */
