...
void create_rlc_bearer_table(sqlite3* db)
{
  ...
  "CREATE TABLE RLC_bearer(tstamp INT CHECK(tstamp > 0)," 
                            ...
                            "txsdu_bytes INT CHECK(txsdu_bytes >= 0),"\
                            ...
                            "rxsdu_bytes INT CHECK(rxsdu_bytes >= 0),"\
...
void insert_db(sqlite3* db, char const* sql)
{
  ...
  for (int attempt = 0; attempt < 50; ++attempt) {
    ...
    fprintf(stderr, "SQLite insert failed rc=%d: %s\nSQL: %.512s\n", rc, err_msg ? err_msg : sqlite3_errmsg(db), sql);
    sqlite3_free(err_msg);
    assert(false && "Error while inserting into the xApp DB");
  }
  fprintf(stderr, "SQLite insert remained busy after 500 ms: %.512s\n", sql);
  assert(false && "xApp DB remained busy");
}
...
void init_db_sqlite3(sqlite3** db, char const* db_filename)
{
  ...
  int const busy_rc = sqlite3_busy_timeout(*db, 5000);
  assert(busy_rc == SQLITE_OK && "Error while setting sqlite3 busy timeout");
