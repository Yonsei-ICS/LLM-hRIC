 ...
 e2ap_msg_t e2ap_handle_control_ack_xapp(e42_xapp_t* xapp, const e2ap_msg_t* msg)
{
  ...
  if (!rv.ok) {
    printf("[xApp]: Ignoring late CONTROL ACK for expired RIC_REQ_ID %d\n", ack->ric_id.ric_req_id);
    e2ap_msg_t ans = {.type = NONE_E2_MSG_TYPE};
    return ans;
  }
 ...
 e2ap_msg_t e2ap_handle_control_failure_xapp(e42_xapp_t* xapp, const e2ap_msg_t* msg)
{
  ...
  if (!rv.ok) {
    printf("[xApp]: Ignoring late CONTROL FAILURE for expired RIC_REQ_ID %d\n", failure->ric_id.ric_req_id);
    e2ap_msg_t ans = {.type = NONE_E2_MSG_TYPE};
    return ans;
  }
...
e2ap_msg_t e2ap_handle_e42_ric_control_request_xapp(e42_xapp_t* xapp, const e2ap_msg_t* msg)
{
  ...
  // control_sm_sync_xapp() already bounds the synchronous wait through
  // cond_wait_sync_ui(). A second timerfd in the pending-event bimap is
  // redundant and corrupts that container after long-running control loops.
