...
void add_reg_e2_node_v1(reg_e2_nodes_t* i, global_e2_node_id_t const* id, size_t len_rf, ran_function_t const* ran_func)
{
  ...
  if(assoc_size(&i->node_to_rf) > 0){
    ...
    if(it_node != end_node) {
      pair_rf_cca_t* old = assoc_extract(&i->node_to_rf, (void*)id);
      free_pair_rf_cca(old);
      free(old);
      fprintf(stderr, "[nearRT-RIC] Replacing already registered E2 Node after duplicate E2 setup\n");
    }
  }
