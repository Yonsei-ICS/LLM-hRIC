...
static void fill_rc_control(ran_func_def_ctrl_t* ctrl)
{
  ...
  fill_rc_ctrl_style_2(&ctrl->seq_ctrl_style[1]);
}
...
//  assert(data->type == RAN_CONTROL_CTRL_V1_03 );
  ...
  LOG_I(NR_MAC,
        "[E2-Agent]: RC CONTROL rx, RIC Style Type %d, Action ID %d\n",
        ctrl->hdr.frmt_1.ric_style_type,
        ctrl->hdr.frmt_1.ctrl_act_id);

  if (ctrl->hdr.frmt_1.ric_style_type == 2
      && ctrl->hdr.frmt_1.ctrl_act_id == RC_CTRL_ACTION_SLICE_LEVEL_PRB_QUOTA) {
    bool ok = apply_rc_ctrl_style_2_slice_level_prb_quota(&ctrl->msg.frmt_1);
    if (!ok)
      LOG_E(NR_MAC, "Failed to apply RC Style 2 Slice-level PRB quota control\n");
    return ans;
  }

  assert(ctrl->hdr.frmt_1.ric_style_type == 1 && ctrl->hdr.frmt_1.ctrl_act_id == 2
         && "Currently only RC Style 1 Action 2 and RC Style 2 Action 6 supported");
